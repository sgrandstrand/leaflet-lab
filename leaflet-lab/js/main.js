//* Javascript sheet by Sarah Grandstrand, 2019 */
// GEOG 575 Lab 1; Avg visitors to National Parks per month

// The average was calculated by taking the monthly vistors for each national       park from 2015-2018
// Information was found at https://irma.nps.gov/Stats/Reports/National  
// Park boundaries were found at                                                    https://catalog.data.gov/dataset/national-parks  
/// Map tileset was found at http://leaflet-extras.github.io/leaflet-               providers/preview/

//function used to create map
function createMap() {
    //create the map, centered map on U.S.
    var map = L.map('map', {
        center: [45, -120],
        zoom: 3,
        zoomControl: false,
...
function processData(data) {
    ...
    for (var attribute in properties) {
        //only take attributes with vistor number values
...
function Popup(properties, attribute, layer, radius) {
    ...
    this.layer = layer;
    this.park = properties.Park
    this.month = attribute.split("_")[0];
    this.visitors = this.properties[attribute];
    this.content = "<p><b>Park:</b> " + this.park + "<br><b>Average Number of Park Visitors in " + this.month + ":</b><br>" + numberWithCommas(this.visitors) + " people." + "</p>";
...
function createSequenceControls(map, attributes) {
    ...
    var SequenceControl = L.Control.extend({
        ...
        onAdd: function (map) {
            ...
            // Header information 
            $(container).append('<h3 align=center id= "header">National Park Visitors Per Month</h3>')
            $(container).append('<p align=center id= "info">Shown is the average number of visitors to national parks every month. <br><br> Below you can click through each month and turn on the park boundaries. Scroll over or click on the circles to see the amount of visitors. Enjoy! </p>')
    ...
    var LegendControl = L.Control.extend({
        ...
        onAdd: function (map) {
            ...
            for (var circle in circles) {
                ...
                svg += '<circle class="legend-circle" id="' + circle + '" fill="#8D9459" fill-opacity="0.8" stroke="#222418" cx="50"/>';
...
function updateLegend(map, attribute) {
    ...
    var content = "<b>Park Visitors in " + month + "</b>";
...
// **** POLYGON BOUNDARY LAYER FUNCTIONS **** //

//load poly data
function getPolyData(map) {
    $.ajax("data/NatParksPoly.geojson", {
        dataType: "json",
        success: function (response) {
...
function stylePoly(feature, layer) {
    layer.bindPopup("<p id=bound-text >" + feature.properties.UNIT_NAME + "</p>");
